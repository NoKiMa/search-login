...
const loginApiSevice = {
   getLoginsData: async (login:string, current_page:string, post_on_page:string) => {
    try { 
        const response = await axios.get(`${BASE_URL}${login}&page=${current_page}&per_page=${post_on_page}`)
