...
const dataSlice = createSlice({
 ...
 extraReducers: (builder)=>{
     builder.addCase(fetchLogin.pending,(reduxState: ReduxState)=>{
            reduxState.status = true
            return reduxState;
           });

    builder.addCase(fetchLogin.rejected, (reduxState: ReduxState)=>{
           
                reduxState.error = "error";
                reduxState.status =  false;
                return reduxState;
        });       
    //  builder.addCase(fetchLogin.fulfilled,(reduxState: ReduxState, action: PayloadAction<Login[]>)=>{
    //     reduxState.state = action.payload;
    //     reduxState.status = false;
    //     return reduxState;
    //     });
